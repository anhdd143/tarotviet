...
import greendream.ait.tarot.animation.AnimationFactory;
import greendream.ait.tarot.animation.AnimationFactory.FlipDirection;
...
import greendream.ait.tarot.view.adapter.AssociationItemViewAdapter;
...
public class SingleCardActivity extends FragmentActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ImageCacheParams cacheParams = new ImageCacheParams(getApplicationContext(), EXTRA_IMAGE_CACHE_DIR);
		...
		btn_card_note = (Button) findViewById(R.id.btn_card_note);
		btn_card_note.setOnClickListener(this);
		btn_card_note.setTypeface(ConfigData.UVNCatBien_R);
	...
	public void onClick(View v) {
		...
		case R.id.btn_card_note:
			Intent intentNote = new Intent(this, EditCardNoteActivity.class);
			intentNote.putExtra(EditCardNoteActivity.EXTRA_CARD_ID, ConfigData.randOneCardId);
			TransitionHelper.transitionTo(this, intentNote);
			break;
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		// Add save to journal option
		menu.add(0, 999, 0, "Lưu lá bài vào nhật ký");
		return true;
	}
	...
	private void saveToJournal() {
		...
		int[] cardIds = new int[]{ConfigData.randOneCardId};
		int[] cardDims = new int[]{ConfigData.randOneCardDimension};
		String cardName = CardsDetailJasonHelper.getEnglishCardName(ConfigData.randOneCardId);
		dbHelper.saveReading(
			-1, // no spread
			cardName != null ? cardName : "Lá bài ngày",
			"", // question
			"", // notes
			cardIds,
			cardDims,
			2 // single card reading type
		);
		android.widget.Toast.makeText(this, "Đã lưu lá bài vào nhật ký", 
